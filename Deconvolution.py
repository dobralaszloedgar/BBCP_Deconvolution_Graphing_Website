...
def run_deconvolution(
        data_array,
        calib_array=None,
        x_axis_type="MW",  # "MW" or "RT"
        x_lim=[1e3, 1e7],  # MW limits or RT limits
        y_lim=[-0.02, 1],
        n_peaks=4,
        plot_sum=False,
        manual_peaks=[],
    ...
    if x_axis_type == "MW":
        ...
        rt_min = mw_to_rt(x_lim[1])  # Higher MW -> Lower RT
        rt_max = mw_to_rt(x_lim[0])  # Lower MW -> Higher RT
        ...
        x_plot = 10 ** f_log_mw(x_rt)
        x_label = x_label if x_label else "Molecular weight (g/mol)"
        ...
        x_plot = x_rt
        x_label = x_label if x_label else "Retention Time (min)"

    # Baseline correction function
    def baseline_correction(x_rt, y, x_plot, method='None'):
        if method == 'None':
    ...
    y_corrected, baseline = baseline_correction(x_rt, y_formatted, x_plot, method=baseline_method)
    ...
    if len(manual_peaks) == 0:
        ...
        for peak in manual_peaks:
            if peaks_are_mw and x_axis_type == "MW":
    ...
    # Calculate percentages and sort peaks
    ...
    if best_fit is not None and len(best_fit) > 0:
        for i, (fit, pct) in enumerate(zip(best_fit, area_percentages)):
            ...
            ax.plot(x_plot, fit, color=peak_colors[i],
    ...
    if best_fit is not None and len(best_fit) > 0:
        if x_axis_type == "MW":
            results_df = pd.DataFrame({
                'Peak': peak_names,
                'Mn (g/mol)': [int(mw) for mw in peak_values],
                'Area %': [round(pct, 1) for pct in area_percentages]
            })
        else:
            results_df = pd.DataFrame({
                'Peak': peak_names,
                'RT (min)': [round(rt, 2) for rt in peak_values],
                'Area %': [round(pct, 1) for pct in area_percentages]
            })
    else:
        if x_axis_type == "MW":
